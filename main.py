...
def onenteremail(bot:EmailBot=None,message:EmailMessage=None):
    ...
    if '/start' in text:
        reply = '💎 Servidor Activado 💎\n\n'
        reply+= '📶 Bot Para Descargar Archivos Desde Internet'
        reply+= '📶 Los Archivos Con Mas De 15mb Se Descargarán En Partes\n\n'
        reply+= '📶 Como Usar?\n'
...
def main():
    natcli = EmailBot(email='alexiszs@nauta.cu',email_password='Kirenia',type='nauta')
    loged = natcli.login()
    if loged:
